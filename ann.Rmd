...
This is an ANN classification algorithm. First of all we implement the functions we use to compose the network.
...
The next function loads the dataset and splits it into the three samples we need: training (60\%), validation (20\%) and test (20\%).
...
The dataset here is composed by 12 features of SSP's (from line 1 to 636) and stars (the remain). 
...
This routine starts the Ws (weight matrices) and biases arrays and train the nn using the training sample and checking its performance on validation dataset.
...
Then we have the BackPropagation function, with the Gradient Descent algorithm. It changes the parameters values on a way that minimizes the loss function.
